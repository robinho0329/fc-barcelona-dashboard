...
다만 <b>이번 평가 구간에서는 순위 신호가 관찰됐다.</b> 모델이 승리 확률을 높게 본
경기와 낮게 본 경기의 실제 승률이 갈렸다. 아래 사분위 표는 이 한 번의 시간순
홀드아웃 결과이며, 다른 시즌에서도 같은 판별력이 이어진다는 보장은 없다.
</div>
""", unsafe_allow_html=True)
...
예측 확률을 실제 승률처럼 해석하면 안 된다. 이번 홀드아웃에서 경기 간 순위 신호가
관찰됐지만 반복 검증은 하지 않았다. 부상·이적·감독 교체·컵대회 일정 같은 맥락은
원본에 없어 반영되지 않는다.
</div>
""", unsafe_allow_html=True)
