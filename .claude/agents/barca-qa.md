...
먼저 스모크 테스트를 돌린다. 페이지를 **실제로 실행**해 예외를 잡는다 —
Streamlit 은 페이지가 예외로 죽어도 HTTP 200 을 주기 때문에 200 만 봐서는
모른다. 등록된 페이지 목록은 app.py 에서 읽으므로 페이지가 늘어도 자동으로 따라간다.

```bash
"/d/workspace/EPL project/.venv/Scripts/python.exe" tools/smoke.py
```

실패하면 종료 코드 1 이다. 그다음 응답 코드도 확인한다.
...
### grep 으로 결론 내지 않는다
**코드에 없다고 화면에 없는 것이 아니다.** 실제로 이런 오판이 있었다 —
`views/masia.py` 가 `credits.json` 을 로드하지 않는 것을 보고
"저작권 표기가 화면에 안 뜬다"고 보고했는데, 사실은 `st.caption` 에
직접 적혀 있어 정상 노출되고 있었다.

화면에 무엇이 뜨는지는 **렌더된 DOM 으로 확인한다.**
...
반대도 마찬가지다. 코드에 있어도 조건 분기에 걸려 안 뜰 수 있다.

### 콘솔 404 두 건은 정상이다
서브페이지 URL 로 직접 들어가면 Streamlit 이 내부 엔드포인트를 **현재 경로
기준**으로 요청해 404 가 뜬다.

    404 /<페이지>/_stcore/health
    404 /<페이지>/_stcore/host-config

앱이 곧 루트로 재시도하므로 무해하다. 실제로 이것을 "선수 사진이 없는 것으로
추정"이라 보고한 적이 두 번 있었는데 둘 다 틀렸다 — 초상 사진은 316/316
전부 있다. **404 를 보면 URL 을 먼저 읽어라.** 이 둘 말고 다른 404 가 있으면
그때가 진짜 문제다.

### 표본을 다시 뽑는 값은 소수점이 흔들린다
부트스트랩·무작위 추출이 들어간 수치는 시드가 다르면 값이 조금 달라진다.
소수점 한두 자리 차이를 불일치로 올리지 말고, **구간이 겹치는지**를 본다.
