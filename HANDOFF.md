...
1. **티키타카 지수** — StatsBomb 패스량·짧은패스 비율 + FBref 전 대회 점유율로
   시즌별 곡선. 2008~2012 정점과 이후 변화를 보여주는 게 목표.
2. **연계 네트워크** — StatsBomb `is_assist`와 Understat `player_assisted`로
   누가 누구에게 어시스트했는지. MSN·차비-이니에스타-메시 삼각형을 그릴 수 있다.
3. **감독별 전술 스타일** — 경기당 슛·코너·파울(2005/06~)과 StatsBomb 패스량·
   압박으로 감독 페이지에 성향 지표 추가.
4. **시대 이미지 2장** — 포스트 과르디올라·재건 칸이 캄 노우 사진으로 대체돼
